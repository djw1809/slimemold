...
def x_prime(t,x):
    ...
    for i in range(len(x[:,1])):
        ...
        output[i, :] =  -np.sum(A_gradW[:,i,:] * M, axis = 1) - np.sum(A_grad_moll[:, i, :]*np.multiply(M,j), axis = 1) - np.power(np.dot(A_moll[i,:],M),m-2) * np.sum(A_grad_moll[:,i,:]*M, axis =1)-10*V_prime(t,x[i, :])
...
def plot_profiles(sol, masses, time, gridpoints):
    global cell_start
    global cell_end
    global root_N
