...
def sweep_2D(ABC_parameters, N, x0, maximum_step_size, T, y0,y1,M, m, eps, kernal_choice, plot_times, filename):
    ...
    for set in ABC_parameters:
        ...
        sol_dict[set] =  sol
        #slime2D.plot_profiles(sol, M, N, param_dict['plot_time'], 200, param_dict['cell_start'], param_dict['cell_end'], param_dict['eps'], filename + "A%d_B%dx_C%d" % (set[0], set[1], set[2]))
