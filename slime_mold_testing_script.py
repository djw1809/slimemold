...
root_N = 1  ## sqrt of number of particles
...
def grad_W(x):
    ##|x|^4/4 - |x| ##
    ##normy = norm(x)
    output = -10*grad_mollifier(x,np.sqrt(.3))#x #(normy**2) * x
    return output
...
def x_prime(t,x,A,B,C):
    ...
    for i in range(len(x[:,1])):
        ...
        output[i, :] =  -B*np.sum(A_gradW[:,i,:] * M, axis = 1) - C*np.sum(A_grad_moll[:, i, :]*np.multiply(M,j), axis = 1) - C*np.power(np.dot(A_moll[i,:],M),m-2) * np.sum(A_grad_moll[:,i,:]*M, axis =1)-A*V_prime(t,x[i, :])
...
def sweep(parameters):
    ...
    for set in parameters:
        #try:
            sol = solve_ivp(lambda t,y:x_prime(t,y,set[0],set[1],set[2]),initial_positions, method = 'BDF', max_step = maximum_step_size)
            sol_dict[set] = sol
        #except:
        #    print("Error with solution parameters A = %d, B = %d, C = %d" % (set[0], set[1], set[2]) )
        #    bad_list.append(set)
        #try:
            plot_trajectories(sol, M, "A%d_B%d,C%d" % (set[0], set[1], set[2]) )
            plot_profiles(sol, M, len(sol['t'])-1, 200, "A%d_B%d,C%d" % (set[0], set[1], set[2]))
        #except:
...
def plot_trajectories(sol, masses, title):
    ...
    plt.savefig(title+"_trajectories.pdf" % time, bbox_inches = 'tight')
    #plt.show()
