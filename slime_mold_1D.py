...
def grad_mollifier(x):
    global eps
    output = -mollifier(x) * np.divide(2, 4*(eps**2)) * x
    return output



def W(x):
    ''' polynomial interaction kernal '''
    output =  mollifier#-x # np.divide(x**4, 4) - np.divide(x**2,2)
    return output
...
def blob(particles, grid, masses):
...
def x_prime(t,x):
    ...
    A = np.zeros([len(x), len(x)]) ###compute all the differences
    for i in range(len(x)):
        A[i, :] = x[i] - x

    A_Wprime = np.vectorize(W_prime)(A)
    A_moll = np.vectorize(mollifier)(A)
    A_moll_prime = np.vectorize(grad_mollifier)(A)
    ...
    for i in range(len(output)):
        ...
        output[i] =    -np.dot(A_Wprime[i, :], M) - np.dot(A_moll_prime[i,:], np.multiply(M, j)) - np.power(np.dot(A_moll[i,:],M),m-2)*np.dot(A_moll_prime[i,:],M) -(V_prime(t,x[i]))

    return output

### compute particle trajectories  - solve x' = f(y,t)###
def solve(method):
    global x0
    global maximum_step_size
    global T
    sol = solve_ivp(x_prime, (0,T), x0, method = method, max_step = maximum_step_size)
    return sol
...
def compute_and_plot_profiles(sol, masses, plot_times, gridpoints):
    global cell_start
    global cell_end
    cmap = plt.get_cmap('viridis')
    fig, ax = plt.subplots(figsize = (7,5))
    ...
    for i in plot_times:
        particles = sol['y'][:,i]
        profile = blob(particles, grid, masses)
...
def plot_both_things(sol,masses,plot_times, gridpoints,title):
    global cell_start
    global cell_end
    global y0
    global y1
    ...
    ax2.set_title('mass profiles colored by timestep')
    fig.suptitle(title)
    t = sol['t']
    traj = sol['y']
    ...
    cb = matplotlib.colorbar.ColorbarBase(ax3, cmap='viridis', norm=cNorm)
    ...
    cb = matplotlib.colorbar.ColorbarBase(ax4, cmap='viridis', norm=cNorm)
